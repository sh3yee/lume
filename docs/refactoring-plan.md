...
| Phase 3：Markdown 扩展 | 已完成 | 建立 WYSIWYG 与 Preview 的成对扩展约定 |
| Phase 4：平台与 Rust | 已完成 | 按领域拆分 Tauri 桥接、命令与后台服务 |
| Phase 5：工作区能力 | 已完成（待桌面手动验收） | 在稳定边界上实现工作区、监听和冲突处理 |
...
- [x] 在上述边界稳定后单独评估 CodeMirror 6 迁移。
...
- [x] 定义内部 Markdown 扩展目录和命名约定。
- [x] 定义每个扩展的 Milkdown 接入点。
- [x] 定义每个扩展的 markdown-it 接入点。
- [x] 定义扩展命令与样式入口。
- [x] 定义扩展的 Markdown 往返手动验收要求。

### 迁移现有能力

- [x] 将基础 Markdown 整理为 `base` 扩展。
- [x] 将表格、任务列表和删除线整理为 `gfm` 扩展。
- [x] 将图片编辑和预览整理为 `images` 扩展。
- [x] 将文档内搜索整理为 `search` 扩展。
- [x] 将代码块交互整理为 `code-block` 扩展。

### 接入路线图能力

- [x] 接入 KaTeX 数学公式编辑与预览。
- [x] 接入 Mermaid 图表编辑与预览。
- [x] 接入脚注编辑与预览。
- [x] 接入高质量代码语法高亮。
- [x] 为所有新增扩展补充[往返和性能手动验收项](markdown-extension-acceptance.md)。

### 完成标准

- [x] 新增 Markdown 能力不需要修改编辑器根组件的核心生命周期。
- [x] 每项语法在 WYSIWYG、源码与 Preview 中具有明确行为。
- [x] 普通键入不因重型扩展同步渲染而明显卡顿。

## Phase 4：平台桥接与 Rust 模块

### 前端平台层

- [x] 将通用 Tauri 调用封装迁移到 `platform/tauri/client.ts`。
- [x] 按文件、图片、工作区和窗口拆分 Tauri API。
- [x] 将浏览器文件降级实现迁移到 `platform/browser`。
- [x] 统一平台错误向业务错误的转换方式。
- [x] 更新开发文档中的 Tauri 桥接路径和新增命令流程。

### Rust 后端

- [x] 将 `commands.rs` 拆分为文件、图片、工作区和系统命令模块。
- [x] 将图片资源存储逻辑提取到 Service。
- [x] 将工作区扫描逻辑提取到 Service。
- [x] 保持所有公开命令使用 `lume_` 前缀和统一错误模型。
- [x] 手动验收路径校验、图片路径转换和工作区扫描行为。
- [x] 确保命令函数保持轻量，耗时能力可迁移至后台任务。

### 完成标准

- [x] Vue 组件中不存在直接的 Tauri `invoke` 或插件调用。
- [x] Rust 命令模块不再混合多个领域的具体实现。
- [x] 前端构建、Rust 检查和用户手动验收通过。

## Phase 5：工作区与外部文件变化

- [x] 建立 Workspace Store 与工作区 Service。
- [x] 实现工作区文件树及折叠状态。
- [x] 实现新建、重命名、移动和删除文档。
- [x] 在 Rust 后台实现文件监听。
- [x] 定义外部修改事件的数据结构和前端处理流程。
- [x] clean 文档发生外部修改时安全刷新。
- [x] dirty 文档发生外部修改时进入冲突状态，不静默覆盖。
- [x] 处理外部删除和重命名。
- [x] 增加源文件自动保存策略和失败反馈。
- [x] 为工作区扫描和文件监听建立性能基线。
...
- [x] 外部修改不会静默覆盖用户未保存内容。
- [x] 工作区扫描与监听不阻塞普通编辑操作。
- [x] 多文档、会话恢复和文件树状态通过用户手动验收。



## 每阶段通用验收清单

- [x] 没有删除仍有价值的现有注释；迁移时同步修正失效注释。
- [x] 没有为目录整齐而创建无实现内容的空模块。
- [x] 没有新增不必要的跨 Feature 内部导入。
- [x] `npm run type-check` 通过。
- [x] `npm run build` 通过。
- [x] Tauri 相关阶段执行 Rust 检查和桌面手动验收。
- [x] 中文输入法连续输入正常。
- [x] 光标、选区、撤销和重做正常。
