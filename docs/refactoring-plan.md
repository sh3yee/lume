...
| Phase 0：应用壳 | 未开始 | 将全局生命周期和用户偏好移出根组件 |
| Phase 1：文档与文件 | 未开始 | 分离状态、会话、业务命令与文件 IO |
...
- [ ] 将主题读取、解析、持久化和系统主题监听迁移到 `app/preferences`。
- [ ] 将窗口尺寸、最大化状态和恢复逻辑迁移到 `app/lifecycle`。
- [ ] 将应用级快捷键注册迁移到 `app/commands`。
- [ ] 将文件和图片拖放分发迁移到独立生命周期模块。
- [ ] 将文档会话持久化调度移出 `App.vue`。
- [ ] 将全局错误反馈统一为应用级反馈入口。
- [ ] 让 `App.vue` 只保留应用壳组合和少量顶层状态连接。

### 完成标准

- [ ] 主题、窗口恢复、快捷键和拖放行为与重构前一致。
- [ ] `App.vue` 不再直接实现平台调用和持久化细节。
- [ ] 构建、类型检查和用户手动验收通过。
...
- [ ] 将文档统计提取为无副作用纯函数。
- [ ] 将会话编解码、版本校验和恢复提取为独立模块。
- [ ] 将会话存储实现与文档内存状态分离。
- [ ] 将文档列表、活动文档、内容和 dirty 状态整理为明确的 Document Store。
- [ ] 将标签关闭确认队列改为职责明确的业务流程。
- [ ] 区分持久化内容状态与编辑器选区、撤销历史等会话状态。

### 文件流程

- [ ] 建立浏览器文件 Adapter。
- [ ] 建立 Tauri 文件 Adapter。
- [ ] 建立文档文件 Service，统一打开、保存和另存为流程。
- [ ] 建立文档 Command，负责组合文件 Service 与 Document Store。
- [ ] 将图片暂存迁移从通用文件操作中移入图片领域服务。
- [ ] 实现“另存为”。
- [ ] 实现关闭应用时的未保存修改保护。

### 完成标准

- [ ] Document Store 不直接访问 `localStorage`、文件系统或 Tauri API。
- [ ] 文件 Adapter 不直接修改 Vue 文档状态。
- [ ] 新建、打开、保存、另存为、关闭标签和退出保护均通过手动验收。
- [ ] 构建和类型检查通过。
